...
def build_app(
    *,
    ...
    agent_exec = A2ALabAgentExecutor(agent=maf_agent)
    ...
    async def _startup() -> None:
        ...
        await maf_agent.__aenter__()          # opens MCP tool

    @app.on_event("shutdown")
    async def _shutdown() -> None:
        log.info("Closing MAFAgent MCP connection …")
        await maf_agent.__aexit__(None, None, None)
...
def _get_agent_card(host_url: str) -> AgentCard:
    ...
    skill = AgentSkill(
        id=f'{TITLE}_forecast_maf',
        name=f'Microsoft Agent Framework {TITLE} forecasting agent',
        description=f'Answers questions about the {TITLE} using the tools provided, breakdown the requests one question at a time.',
        tags=[f'{TITLE}', 'microsoft-agent-framework'],
        examples=[
    ...
    return AgentCard(
        name=f'MAF {TITLE} Agent',
        description=f'Microsoft Agent Framework-powered {TITLE} agent',
        url=f'{host_url}',
        version='1.0.0',
